...
START_TEST (test_translation)
    {
        znat_t *nat = znat_new(0);
        // x.x.x.x:1234 -> 8.8.8.8:53
        znat_origin_t origin = {1234, htons(53), 0x08080808};
        uint16_t port = znat_translate(nat, PROTO_TCP, &origin);
        fail_if(0 == port, "NAT translation returned zero port");

        znat_origin_t found_origin;
        fail_if(!znat_lookup(nat, PROTO_TCP, port, &found_origin), "NAT lookup failed");
        ...
        fail_if(znat_lookup(nat, PROTO_UDP, 31337, &found_origin), "NAT lookup found untranslated entry");
