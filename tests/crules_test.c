...
START_TEST (test_deferred)
    {
        const struct zrule_deferred test_data[] = {
                {125, "foo"},
                {1, "bar"},
        };
        ...
        fail_if(0 != crules_parse(&rules, "deferred.125.foo"), "parse fail");
        fail_if(0 != crules_parse(&rules, "deferred.1.bar"), "parse fail");
...
START_TEST (test_shape)
    {
        ...
        fail_if(!rules.have.user_id, "shape have user_id fail");
        fail_if(!rules.have.bw_down, "shape have bw_down fail");
        fail_if(31337 != rules.user_id, "shape user_id value fail");
        fail_if(262144 != rules.bw_down, "shape out value fail");
...
START_TEST (test_login)
    {
        ...
        fail_if(0 != crules_parse(&rules, "login.ababa"), "login parse fail");
        fail_if(!rules.have.login, "login have login fail");
        fail_if(0 != strcmp("ABABA", rules.login), "login login value fail");

        crules_free(&rules);
    }
END_TEST

START_TEST (test_mark_policer)
    {
        struct zcrules rules;
        crules_init(&rules);
...
START_TEST (test_no_smtp)
    {
        struct zcrules rules;
        crules_init(&rules);

        fail_if(0 != crules_parse(&rules, "no_smtp"), "no_smtp parse fail");
        fail_if(!rules.have.port_rules, "no_smtp have port_rules fail");

        crules_free(&rules);
    }
END_TEST

START_TEST (test_dns_filter_type)
    {
        struct zcrules rules;
        crules_init(&rules);

        fail_if(0 != crules_parse(&rules, "dns_filter_type.0"), "dns_filter_type parse fail");
        fail_if(!rules.have.port_rules, "dns_filter_type have port_rules fail");
        fail_if(!rules.have.fwd_rules, "dns_filter_type have fwd_rules fail");

        crules_free(&rules);
        crules_init(&rules);

        fail_if(0 != crules_parse(&rules, "dns_filter_type.1"), "dns_filter_type parse fail");
        fail_if(!rules.have.port_rules, "dns_filter_type have port_rules fail");
        fail_if(!rules.have.fwd_rules, "dns_filter_type have fwd_rules fail");
        ...
        fail_if(0 != crules_parse(&rules, "dns_filter_type.3"), "dns_filter_type parse fail");
        fail_if(!rules.have.port_rules, "dns_filter_type have port_rules fail");
        fail_if(!rules.have.fwd_rules, "dns_filter_type have fwd_rules fail");
...
Suite *create_test_suite()
{
    ...
    tcase_add_test(tcase, test_no_smtp);
    tcase_add_test(tcase, test_dns_filter_type);
